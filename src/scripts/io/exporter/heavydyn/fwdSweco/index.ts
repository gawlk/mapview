...
export class FWDSwecoExportStrategy implements ExportStrategy {
  ...
  doExport(project: HeavydynProject): string {
    return '\n' + this.writeHeader(project) + this.writePoints(project) + '\n'
  }
  ...
  private writeHeader(project: HeavydynProject): string {
    ...
    return dedent`
            ...
            ${
              this.padDotString('Radial offset [cm]', 23) +
              sensors
                .map((channel) =>
                  Math.round(parseFloat(channel.position) * 100)
                )
                .join('\t')
            }
            ${
              this.padDotString('Tolerance [%]', 23) +
              sensors.map(() => '5').join('\t')
            }
            ${
              this.padDotString('Correction [%]', 23) +
              sensors.map(() => '0').join('\t')
            }
  ...
  private writeDrops(point: MachinePoint, channels: JSONChannel[]): string {
    ...
    return dedent`
            ...
            ${dropHeader.join('\t')}
            ${point.drops
              .map((drop) => this.writeDrop(drop, channels))
              .join('\n')}
        `
  }
